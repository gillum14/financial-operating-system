...
// Drag-and-drop reordering is client-side/session-only — there is no
// sortOrder column on the categories table to persist it to (a schema
// migration wasn't authorized for this pass; see the delivery report's
// backend gaps). `rows` is re-seeded from `overview` whenever the server
// view changes (e.g. after any create/edit/archive triggers
// revalidatePath("/categories")), so a reordered list reverts to
// persisted (name) order on the next mutation or refresh — expected, not
// a bug.
export function CategoriesWorkspace({ overview }: { overview: CategoriesOverviewView }) {
  ...
  function handleTopLevelDrop(targetId: string) {
    ...
    setRows((prev) => {
      const fromIndex = prev.findIndex((row) => row.category.id === draggedTopLevelId);
      const toIndex = prev.findIndex((row) => row.category.id === targetId);
  ...
  function handleSubcategoryDrop(parentId: string, targetId: string) {
    ...
    setRows((prev) =>
      prev.map((row) => {
        if (row.category.id !== parentId) return row;
        const fromIndex = row.subcategories.findIndex((sub) => sub.category.id === dragged.id);
        const toIndex = row.subcategories.findIndex((sub) => sub.category.id === targetId);
        if (fromIndex === -1 || toIndex === -1) return row;
        const nextSubcategories = [...row.subcategories];
        const [moved] = nextSubcategories.splice(fromIndex, 1);
        nextSubcategories.splice(toIndex, 0, moved);
        return { ...row, subcategories: nextSubcategories };
      }),
    );
  }
        ...
        <div className="min-w-0 space-y-6">
          ...
          {rows.length > 0 && (
            <p className="text-xs text-[var(--foreground-muted)]">
              Drag and drop categories or subcategories to reorder them. Order is not saved between visits.
            </p>
          )}
        </div>
