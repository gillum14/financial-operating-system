...
// .../budgets/error.tsx, .../goals/error.tsx, and .../reports/error.tsx.
export default function MissionsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("Missions page failed to load:", error);
  }, [error]);

  return (
    <Card className="text-center">
      <p className="text-lg font-semibold text-[var(--foreground)]">Unable to load Missions</p>
